...
/// struct for typed errors of method [`get_album_info_by_name`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetAlbumInfoByNameError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_album_info_by_name(configuration: &configuration::Configuration, album_name: &str) -> Result<crate::models::AlbumResponseDto, Error<GetAlbumInfoByNameError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/album/name/{albumName}", local_var_configuration.base_path, albumName=crate::apis::urlencode(album_name));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetAlbumInfoByNameError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
